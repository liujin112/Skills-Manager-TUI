...
pub struct PresetsView {
    ...
    color_prompt: Option<super::tags::Prompt>,
}
...
impl PresetsView {
        ...
        let rendered: Vec<_> = self
            ...
            .map(|(i, (name, keys))| {
                ...
                cards::Pill {
                    ...
                    ..cards::Pill::new(name, cards::tag_fill(name, ctx))
                }
                .render(ctx, budget)
            })
            .collect();
    ...
    fn draw_presets(&mut self, f: &mut Frame, area: Rect, ctx: &Ctx) {
        ...
        let heights = self
            .presets
            .iter()
            .map(|p| 3 + u16::from(p.description.as_ref().is_some_and(|s| !s.trim().is_empty())))
            .collect();
    ...
    fn draw_members(&mut self, f: &mut Frame, area: Rect, ctx: &Ctx) {
        ...
        let title = Line::from(vec![
            ...
            Span::styled(
                format!(" · {} skills ", self.visible_members.len()),
                th.skill_count(),
            ),
        ]);
        ...
        match k.code {
            ...
            KeyCode::Char('C') => {
                if let Some(p) = self.selected() {
                    self.color_prompt =
                        Some(super::tags::Prompt::for_color(&p.name, p.color.as_deref()));
                }
                vec![]
            }
            KeyCode::Char('a') => self.add_members(ctx),
